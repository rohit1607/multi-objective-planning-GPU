...
if __name__ == "__main__":
    ...
    filename_part1 = '/gdata1/rohitc/e2e_GPU_DP/src/data_solverOutput/custom2/DG3_g100x100x120_r5k_vmax6_2LpDynObs_0.7ObsVel/a2x16_i15_j40_ref2'
    ...
    # print("Time: ")
    # for i in range(21):
    #     filename = filename_part1 + '/' + '{0:.6f}' + '/interp_traj_data.npy'
    #     data = np.load(filename.format(i/20), allow_pickle=True)
    #     time_reqd = data[:,3]
    #     mean_time = np.mean(time_reqd)
    #     variance = np.std(time_reqd)
    #     max = np.max(time_reqd)
    #     min = np.min(time_reqd)
    #     print("alpha: ", i/20, " mean: ", mean_time, " std: {0:.2f}".format(variance), " max: ", max, " min: ", min)
    ...
    #     max = np.max(time_reqd)
    #     min = np.min(time_reqd)
    #     print("alpha: ", i/20, " mean: {0:0.3f}".format(mean_time), " std: {0:.2f}".format(variance), " max: {0:0.3f}".format(max), " min: {0:0.3f}".format(min))
    ...
    for i in range(21):
        filename = filename_part1 + '/' + '{0:.6f}' + '/interp_traj_data.npy'
        data.append(np.load(filename.format(i/20), allow_pickle=True))
        arr_alpha.append(i/20)
    ...
    for i in range(21):
        time.append(data[i][:,3].mean())
        net_energy.append(data[i][:,6].mean())
    plt.scatter(time, net_energy, marker = 'o', c=arr_alpha, cmap=cmap)
    plt.plot(time, net_energy, 'k')

    
    filename = "scatter_pareto.png"
    plt.show()
    ...
    #     time_reqd = data[:,3]
    #     net_energy = data[:,6]
    #     cov = covariance(time_reqd.tolist(),net_energy.tolist())
    #     print("alpha: ", i/20, " covariance: {0:0.3f}".format(cov))
