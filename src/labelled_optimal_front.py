...
if __name__ == "__main__":
    ...
    for i in np.linspace(logStart, logEnd, (logEnd-logStart+1), dtype=int):
        ...
        arr_energy[i-logStart] = data['reqd_energy_mean'][i-2]  
    ...
    plt.plot(arr_time, arr_energy)
    plt.xlabel("Avg. Time Required")
    plt.ylabel("Avg. Energy Required")
    plt.title("Pareto-Optimal Front for Time-Energy")
    plt.colorbar(cmap=cmap, orientation="horizontal", ticks=bounds2, boundaries=bounds)
    plt.xlim(27, 65)
    plt.ylim(40,135)
    ...
    print(filepath)
    filename = "pareto_" + str(start) + "_" + str(delta) + "_" + str(end) + "_test_new_scaled" + ".png"
    print(filename)
    plt.show()
